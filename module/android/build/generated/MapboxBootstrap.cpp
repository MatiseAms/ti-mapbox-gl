...
#define TAG "matise.mapbox"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Mapbox_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Mapbox.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::MapboxBindings::lookupGeneratedInit(
...
static void Mapbox_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Mapbox_getBinding)->GetFunction());
}

static void Mapbox_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::MapboxBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry MapboxBinding = {
	"matise.mapbox",
	Mapbox_init,
	Mapbox_dispose
};
...
Java_matise_mapbox_MapboxBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("matise.mapbox", &MapboxBinding);
	titanium::KrollBindings::addExternalLookup(&(::MapboxBindings::lookupGeneratedInit));
}
